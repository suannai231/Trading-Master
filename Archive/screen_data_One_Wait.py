...
def screen(df):
    ...
    if (close>=ema5) and (ema5 >= ema10) and (ema10 >= ema20) and (ema20 >= ema60) and (OBV_MAX>OBV>=OBV_MAX*0.85) and (turnover >= 100000):
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    screened_data_file = datetime_str + '_wait.csv'
    ...
    return_df.to_csv(screened_data_path + datetime_str + '_wait.csv')
    return_df.loc[return_df.date==str(end),'ticker'].to_csv(screened_data_path + datetime_str + '_wait.txt',header=False, index=False)