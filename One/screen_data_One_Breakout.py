...
def screen(df):
    ...
    if (close>=ema5) and (ema5 >= ema10) and (ema10 >= ema20) and (ema20 >= ema60) and (OBV==OBV_MAX) and (turnover >= 100000):
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    return_df.ticker.to_csv(screened_data_path + datetime_str + '_breakout.txt',header=False, index=False)