...
def screen(df):
    ...
    if turn_Indicator & ((df['turn'][lastindex] > TURN_75) | (df['turn'][lastindex] < TURN_25)):
        return pd.DataFrame()
    ...
    if OBV_Indicator & (obv_above_zero_days > OBV_ABOVE_ZERO_DAYS_75) | (obv_above_zero_days < OBV_ABOVE_ZERO_DAYS_25):
        return pd.DataFrame()

    if OBV_Indicator & ((df['OBV_DIFF_RATE'][lastindex] > OBV_DIFF_RATE_75) | (df['OBV_DIFF_RATE'][lastindex] < OBV_DIFF_RATE_25)):
        return pd.DataFrame()

    if Cum_Turnover_Indicator & ((df['cum_turnover'][lastindex] > CUM_TURNOVER_RATE_75) | (df['cum_turnover'][lastindex] < CUM_TURNOVER_RATE_25)):
        return pd.DataFrame()
    ...
    if WR_Indicator & ((df.WR34[lastindex] > WR34_75) | (df.WR34[lastindex] < WR34_25)):
        return pd.DataFrame()

    if WR_Indicator & ((df.WR34[lastindex] > WR120_75) | (df.WR34[lastindex] < WR120_25)):
        return pd.DataFrame()
    ...
    if WR_Indicator & ((wr120_less_than_50_days > WR120_GREATER_THAN_50_DAYS_75) | (wr120_less_than_50_days < WR120_GREATER_THAN_50_DAYS_25)):
        return pd.DataFrame()
    ...
    if WR_Indicator & ((wr120_less_than_80_days > WR120_GREATER_THAN_80_DAYS_75) | (wr120_less_than_80_days < WR120_GREATER_THAN_80_DAYS_25)):
        return pd.DataFrame()
